    ...
    for batch in range(batchSize):
        ...
        for _ in range(100):
            ...
            for j in range(1, model.treeNum+3):
                ...
                loss = loss + Coeff_r(model.treeNum,j)*mc.integrate(lambda x:(func(x))**2,dim,5000,domain)
            loss = loss + 0.1*model.treeNum**(-4)*mc.integrate(lambda x:(LaplaceOperator(lambda s:model.treeDict[str(model.treeNum-1)](s),x))**2,dim,5000,domain)
            print(_,loss)
            loss.backward()
            optimizer.step()
            with torch.no_grad():
        ...
        optimizer = torch.optim.LBFGS(model.treeDict.parameters(), lr=2, max_iter=30)
        ...
        def closure():
            ...
            for j in range(1, model.treeNum+3):
                func = lambda x:(sum([Coeff(j,n,T,'a',1)*model.treeDict[str(n-1)](x) - Coeff(j,n,T,'b',1)*LaplaceOperator(lambda \
                            s:model.treeDict[str(n-1)](s),x) for n in funcCoeffListGen(j, model.treeNum,1)]) - integration1DforT(
                                lambda s,l:f(s,l)*Psi(order, j, T)(l),T,x))
                loss = loss + Coeff_r(model.treeNum,j)*mc.integrate(lambda x:(func(x))**2,dim,2000,domain)
            loss = loss + 0.1*model.treeNum**(-4)*mc.integrate(lambda x:(LaplaceOperator(lambda s:model.treeDict[str(model.treeNum-1)](s),x))**2,dim,2000,domain)
            print(loss)
            loss.backward()
            with torch.no_grad():
                outputFunc = lambda x,t: sum([model.treeDict[str(j)](x)*Phi(order,j+1,T)(t) for j in range(model.treeNum)])
