...
from BinaryTree import ShowTree
import random
...
def TreeTrain(f, model, batchOperations, domain, T, dim, order, real_func):
    ...
    for batch in range(batchSize):
        ...
        model.tree.OperatorsGen(model.tree.tree,batchOperations[batch])
        ...
        for _ in range(0):
            ...
            for j in range(1, model.outNum+3):
                tempLoss = bl.integrate(lambda x:((intFunc(model.tree, x, j, T, order))**2),dim,10000,domain) - \
                        bl.integrate(lambda xt:2*(intFunc(model.tree, xt[:,:-1], j, T, order)*(f(xt)*(Psi(order,j,T)(xt[:,-1])).view(-1,1))),dim+1,10000,domainT)
